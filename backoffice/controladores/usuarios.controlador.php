			    ...
			    if($respuesta == "ok")
				    {
						...
						echo '<script>
							swal({
								...
								text: "¡Ya puede ingresar digitando el correo electrónico y contraseña que acaba de registrar, e iniciar a crear su Modelo de Negocio!",
		 ...
		 if($respuesta["email"] == $_POST["ingresoEmail"]  && $respuesta["password"] == $encriptar)
		 {
		 	...
		 	echo '<script>
				swal({
					type:"error",
					title: "¡ERROR!",
					text: "¡El correo electrónico no ha sido verificado, por favor revisa la bandeja de entrada o la carpeta SPAM de su correo electrónico para verificar la cuenta o contáctese con nuestro soporte a info@gmail.com !",
					showConfirmButton: true,
					confirmButtonText: "Cerrar"
					}).then(function(result){
					if(result.value){
...
function generarPassword($longitud){
	...
	$mail->Charset = "UTF-8";
	$mail->isMail();
	$mail->setFrom("info@modelo-de-negocio.com", "Modelo De Negocio");
	$mail->addReplyTo("info@modelo-de-negocio.com", "Modelo De Negocio");
	$mail->Subject  = "Solicitud nuevo password";
	...
	$mail->msgHTML('<div style="width:100%; background:white; position:relative; font-family:sans-serif; padding-bottom:40px">				
				<div style="position:relative; margin:auto; width:600px; background:#eee; padding:20px">		
					...
					<div style="line-height:30px; background:#0aa; width:60%; padding:20px; color:white">		Haz click aquí
					</div>
					</a>
				...
				<div style="background:#000000;">
        			<center>
        				<img style="padding:20px; width:20%" src="https://modelo-de-negocio.com/vistas/img/logo-medelo-de-negocio.png">
        			</center>
        			</div>
			</div>');								
		$envio = $mail->Send();
...
public function ctrActualizarPerfilUsuario()
{
	if(isset($_POST["nombreUsuario"]))
	    ...
	    if(preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["nombreUsuario"]) &&
			...
			if($respuesta == "ok")
			 {
			 	echo '<script>
							swal({
								type:"success",
								title: "¡OK!",
								text: "¡Su perfil se actualizo correctamente! '.$respuesta.'",
								showConfirmButton: true,
								confirmButtonText: "Cerrar"
								}).then(function(result){
								if(result.value){
